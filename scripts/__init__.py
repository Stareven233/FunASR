...
def parse_transcribe_args():
  ...
  parser.add_argument(
    ...
    default=False,
    help=(
      'allow hub download / remote validation when the local model_zoo cache '
      'is missing or incomplete (default: off; use cache-only / offline load)'
    ),
  )
  parser.add_argument(
    '--check-latest',
    ...
    default=False,
    help=(
      'contact the hub to verify the cached model is the latest revision '
      '(default: off; skip remote freshness checks)'
    ),
  )
  parser.add_argument(
    '--model-revision',
    ...
    default=None,
    help=(
      'pinned model revision / commit for hub resolve. Default: None for HF '
      '(repo default branch), "master" for ModelScope when needed'
    ),
  )
  parser.add_argument(
    '--nano-batch-mode',
    type=str,
    choices=['timestamps', 'fast', 'sequential'],
    default='timestamps',
    help=
    (
      'Fun-ASR-Nano multi-segment LLM batch policy when CTC is loaded '
      '(default: timestamps). timestamps = one batched generate + per-segment '
      'CTC forced alignment (SRT/ASS safe); fast = batched generate only, '
      'subtitle timing falls back to VAD ranges; sequential = one generate '
      'per VAD segment (debug / regression oracle)'
      'timestamps：有加速，CTC 仍按段跑（相对 LLM 很轻），字幕时间戳正常。'
      'fast：和 timestamps 一样 batched LLM，少做 CTC；字幕只能用 VAD 段起止当 cue，不是字级时间。'
      'sequential：无 batch 加速，最慢，但行为最接近旧逻辑。'
    ),
  )
